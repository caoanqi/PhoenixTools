package com.phoenix.myapplication.utils;
...
import com.phoenix.myapplication.PhoenixApp;
...
public class DrawableUtils {
    ...
    public static GradientDrawable getGradientDrawable(int fillColor, int strokeColor, int roundRadius, int strokeWidth) {
        ...
        drawable.setColor(PhoenixApp.getInstance().getResources().getColor(fillColor));
        if (strokeColor > 0) {
            drawable.setStroke(CommonUtils.Dp2Px(PhoenixApp.getInstance(), strokeWidth), PhoenixApp.getInstance().getResources().getColor(strokeColor));
        }
        drawable.setCornerRadius(CommonUtils.Dp2Px(PhoenixApp.getInstance(), roundRadius));
        return drawable;
    }

    public static Drawable getCircleDrawable(@ColorRes int normalColor, @ColorRes int pressedColor, @DimenRes int roundRadius) {
        int size = CommonUtils.Dp2Px(PhoenixApp.getInstance(), roundRadius);
        StateListDrawable bg = new StateListDrawable();
        GradientDrawable drawableNor = new GradientDrawable();
        drawableNor.setSize(size, size);
        drawableNor.setCornerRadius(size / 2);
        drawableNor.setColor(PhoenixApp.getInstance().getResources().getColor(normalColor));
        ...
        drawablePrressed.setColor(PhoenixApp.getInstance().getResources().getColor(pressedColor));
    ...
    public static StateListDrawable getStateListDrawable(@DrawableRes int idNormal, @DrawableRes int idPressed, @DrawableRes int isEnabled) {
        StateListDrawable bg = new StateListDrawable();
        Drawable normal = idNormal == -1 ? null : PhoenixApp.getInstance().getResources().getDrawable(idNormal);
        Drawable pressed = idPressed == -1 ? null : PhoenixApp.getInstance().getResources().getDrawable(idPressed);
        Drawable enabled = isEnabled == -1 ? null : PhoenixApp.getInstance().getResources().getDrawable(isEnabled);
        // View.PRESSED_ENABLED_STATE_SET
    ...
    public static StateListDrawable getStateSelectDrawable(@DrawableRes int selectRes, @DrawableRes int defaultRes) {
        StateListDrawable bg = new StateListDrawable();
        Drawable normal = defaultRes == -1 ? null : PhoenixApp.getInstance().getResources().getDrawable(defaultRes);
        Drawable select = selectRes == -1 ? null : PhoenixApp.getInstance().getResources().getDrawable(selectRes);
    ...
    public static StateListDrawable getStateListDrawable(@ColorRes int colorNormal, @ColorRes int colorPressed,
        ...
        Resources resources = PhoenixApp.getInstance().getResources();
    ...
    private static LayerDrawable createDrawable(@ColorInt int colorPressed, @ColorInt int colorNormal,
        ...
        //对某个内部的资源manage
        layerDrawable.setLayerInset(1, 0, 0, CommonUtils.Dp2Px(PhoenixApp.getInstance(), 1.3f), CommonUtils.Dp2Px(PhoenixApp.getInstance(), 1.8f));
    ...
    public static Drawable createRipple(@ColorRes int rippleColor, @ColorRes int normal, @ColorRes int pressed) {
        Drawable normalDrawable = new ColorDrawable(PhoenixApp.getInstance().getResources().getColor(normal));
        Drawable pressedDrawable = new ColorDrawable(PhoenixApp.getInstance().getResources().getColor(pressed));
    ...
    public static Drawable createRipple(@ColorRes int rippleColor, Drawable normalDrawable, Drawable pressedDrawable) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ColorStateList colorList = ColorStateList.valueOf(PhoenixApp.getInstance().getResources().getColor(rippleColor));
